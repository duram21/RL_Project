...
def main():
    ...
    # -------------------------------------------------
    # 1. 상태 (testing-tool과 동기화할 내부 상태)
    # -------------------------------------------------
    my_hand  = []               # 내 손패 (tile id 리스트)
    opp_hand = []               # 상대 손패 (대충 추적)
    board    = [-1] * num_cells # 각 칸의 tile id, 없으면 -1

    env_sim = None
    model   = None

    # -------------------------------------------------
    # 2. ActorCritic (이 env에 맞춰 학습한 ResNet 구조와 동일)
    # -------------------------------------------------
    def build_model_from_env(env):
        import torch.nn as nn
        import torch.nn.functional as F

        obs_dim = env.obs_dim      # 1184
        act_dim = env.action_size  # 320
        ...
        class ActorCriticImpl(nn.Module):
            """
            - obs: [B, 1184]
              * 앞 1024: board (64*16 one-hot)
              * 다음 80:  내 손패 (5*16)
              * 다음 80:  상대 손패 (5*16)
            - act_dim: 320 (5 슬롯 * 64 칸)
            - env._opponent_model_move에서
              forward_shared, actor_conv, actor_bn, actor_fc 를 사용하므로
              그 이름을 그대로 유지.
            """
            def __init__(self, obs_dim: int, act_dim: int):
                ...
                # delta_fc (aux head, 학습만; ckpt에 키 이름 맞추기용)
            ...
            def forward_shared(self, obs):
                # env._opponent_model_move 에서 사용
                return self.encode_board_hand(obs)
    ...
    def select_action_pure_ppo():
        """env_sim의 상태를 (board, my_hand, opp_hand)로 맞추고 greedy action 선택"""
    ...
    while True:
        ...
        if cmd == "READY":
            ...
            if model is None:
                import torch as t
                import numpy as n
                # ⬇️ 위에 올린 ConnexionEnv 코드가 들어있는 파일 이름에 맞게 수정해
                # 예: from connexion_env_new import ConnexionEnv as CE
                from connexion_env_6 import ConnexionEnv as CE
