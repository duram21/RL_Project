...
class Game:
    ...
    def calculateMove(self, _myTime: int, _oppTime: int) -> Tuple[Cell, Tile]:
        ...
        인자 목록
        - _myTime (int): 내 프로그램의 남은 시간
        - _oppTime (int): 상대 프로그램의 남은 시간
        """

        bestMove: Optional[Tuple[Cell, Tile]] = None
        bestScore = 0

        # 가능한 모든 행동중 최대 점수를 찾음
        for t in self.myTiles:
            for c in Cell.getAllCells():
                # 이미 배치된 칸인지 확인
                if c not in [c for c, _ in self.board]:
                    move = (c, t)

                    # 점수를 계산 후 최고 점수를 갱신
                    score = self.calculateScore(self.board + [move], self.isFirst)
                    if score > bestScore:
                        bestScore = score
                        bestMove = move
